...
3.  Клонируем свою копию репозиторий на компьютер с помощью команды `git clone <url репозитория>`;
...
Внутри директории, соответствующей теме практической работы, должны размещаться `.js`-файлы с выполненным заданием. Изначально директория содержит только файл README.md, в котором содержится описание для практической работы.

**Каждая задача в практической работе должна быть выполнена в отдельном файле, который называется по имени задачи, прим. `task-01.js`, где `01` - номер задачи (если практическая работа подразумевает выполнение только одной задачи файл следует назвать `task.js`). Функция, являющийся решением задачи должна быть экспортирована из файла решения с помощью, инструкции `module.exports = <имя_функции>;`.**

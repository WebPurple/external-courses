...
**Каждая задача в практической работе должна быть выполнена в отдельном файле, который называется по имени задачи, прим. `task-01.js`(task-01.html), где `01` - номер задачи (если практическая работа подразумевает выполнение только одной задачи, файл следует назвать `task.js`). Функция, являющаяся решением задачи, должна быть экспортирована из файла решения с помощью инструкции `module.exports = <имя_функции>;`.**
